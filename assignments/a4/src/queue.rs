...
impl<TaskType: 'static + Task + Send> WorkQueue<TaskType> {
    pub fn new(n_workers: usize) -> WorkQueue<TaskType> {
        ...
        // unimplemented!()
        let mut _worker: Vec<thread::JoinHandle<()>> = Vec::with_capacity(n_workers);
        let (spmc_sender, spmc_reciever) = spmc::channel();
        let (mpsc_sender, mpsc_reciever) = mpsc::channel();

        for _ in 0..n_workers {
            let _sender = mpsc_sender.clone();
            let _reciever = spmc_reciever.clone();
            _worker.push(thread::spawn(move || {WorkQueue::run(_reciever, _sender)}));
        }

        let _result = WorkQueue {send_tasks: Some(spmc_sender), recv_tasks: spmc_reciever, recv_output: mpsc_reciever, workers: _worker};
    ...
    pub fn enqueue(&mut self, t: TaskType) -> Result<(), spmc::SendError<TaskType>> {
        ...
        match self.send_tasks {
            None => {panic!();},
            Some(ref mut send_tasks) => {return send_tasks.send(t);}
        }
    }
    ...
    pub fn shutdown(&mut self) {
        ...
        loop {
            let _result = self.recv_tasks.recv();
            match _result {
                Err(_) => {break;},
                Ok(_) => {continue;}
            }
        ...
        let _worker = self.workers.drain(..);
        for w in _worker {
            w.join().unwrap();
        }
    }
